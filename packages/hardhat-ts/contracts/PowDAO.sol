...
contract PowDAO is ReentrancyGuard {
  // GLOBAL CONSTANTS
  // ***************
  ...
  constructor(address[] memory approveMembers) {
    for (uint256 i = 0; i < approveMembers.length; i++) {
      members[approveMembers[i]] = Member(1, 0, true, 0);
    }

    summoningTime = block.timestamp;
    periodDuration = 17280; // 5 periods/ day = 17280, 35 periods/day = 102.85714
    votingPeriodLength = 180; // 7 days = 604800, 1 hr = 3600
  }
  ...
  // function getPayout(address payable addressOfProposer) public nonReentrant returns (bool) {
  ...
  function getPayoutUnsafe(address payable addressOfProposer) public returns (bool) {
    ...
    if (allowanceAvailable != 0 && allowanceAvailable > 0) {
      (bool success, ) = addressOfProposer.call{ value: _payoutTotals[addressOfProposer] }("");
      require(success, "Transaction fail");
  ...
  function _cancelPropposal(uint256 proposalId) internal onlyMember {
  ...
  function submitVote(uint256 proposalId, uint8 uintVote) public onlyMember {
    ...
    if (vote == Vote.Yes) {
      prop.yesVotes += member.shares;
    } else if (vote == Vote.No) {
      prop.noVotes += member.shares;
    }
